...
import { TODAS, QUALIDADE_LIVRE, ehDoBerserk, qualidadesDe, qualidadeValida, prioridadeDe } from './qualidades.ts';
...
test('1080p e 60 quadros são do Turbo, em qualquer combinação', () => {
  assert.equal(ehDoBerserk('720p30'), false);
  assert.equal(ehDoBerserk('1080p30'), true, '1080p pede Turbo mesmo a 30');
  assert.equal(ehDoBerserk('720p60'), true, '60 quadros pede Turbo mesmo em 720p');
  assert.equal(ehDoBerserk('1080p60'), true);
});
